...
namespace SearchPlusPlus
{
    public class ModMain : MelonMod
    {
        ...
        {
            if (isFirstRun && sceneName == "Loading")
            {
                try
                {
                    isFirstRun = false;
                    MelonLogger.Msg("Checking charts for cinemas, this may take a while...");
                ...
                catch (Exception ex)
                {
                    ...
                    MelonLogger.Warning("If you're seeing this, then I have absolutely 0 clue how. Either way, cinema tag won't work. (e.g. please report lmao)");
                }
        ...
        public override void OnLateInitializeMelon()
        {
            ...
            try
            {
                response = Get(SearchPatch.advancedJsonUrl);
            ...
            {
                MelonLogger.Error(ex.ToString());
                MelonLogger.Msg(ConsoleColor.DarkRed, "Failed to recover advanced search results. If you're seeing this, this is a critical error and should be reported immediately.");
            }
        ...
        private static bool TryParseCinemaJson(Album album)
        {
    ...
    internal static class MenuPatch
    {
        internal static void Prefix()
        {
            ...
            MelonLogger.Msg("Parsed tags: ß" + string.Join(" ", SearchPatch.tagGroups.Select(x1 => string.Join("|", x1.Select(x2 => $"{x2.Key}:\"{x2.Value}\"")))) + 'ß');
            return;
            breakLoop:
            SearchPatch.isAdvancedSearch = true;
            NullifyAdvancedSearch();
    ...
    internal static class SearchPatch
    {
        ...
        private static Dictionary<string, int> validFilters = new Dictionary<string, int> 
        {
            ["diff"] = 1,
            ["bpm"] = 1,
            //["cinema"] = false,
        ...
        private static HashSet<string> allowsWildcard = new HashSet<string>
        {
            "diff",
            "bpm",
            "hidden",
        ...
        private static Dictionary<string, string> validScenes = new Dictionary<string, string>
        {
            {"spacestation", "01"},
            {"space_station", "01"},
            {"retrocity", "02"},
        ...
        private static bool Prefix(ref bool __result, PeroString peroString, MusicInfo musicInfo, string containsText)
        {
        ...
        private static bool LowerContains(this PeroString peroString, string compareText, string containsText)
        {
        ...
        internal static bool CheckFilter(KeyValuePair<string, string> filter, out string errors)
        {
            ...
            {
                errors = "runtime error: received key was null";
                return false;
            }
            ...
            {
                errors = $"runtime error: received unknown key \"{key}\"";
                return false;
            }
            switch (validFilters[key])
            {
                ...
                case -1:
                    if (value == "")
                    {
                        errors = $"runtime error: range-type key \"{key}\" received empty value";
                        return false;
                    }
                    if (value != null && !EvalRange(value, out _, out _) && !(allowsWildcard.Contains(key) && value == "?"))
                    {
                        errors = $"runtime error: failed to parse value \"{value}\" for range-type key \"{key}\"";
                        return false;
                    }
                    break;
                case 0:
                ...
                case 1:
                    ...
                    {
                        errors = $"runtime error: range-type key \"{key}\" received empty or null value";
                        return false;
                    }
                    if (!EvalRange(value, out _, out _) && !(allowsWildcard.Contains(key) && value == "?"))
                    {
                        errors = $"runtime error: failed to parse value \"{value}\" for range-type key \"{key}\"";
                        return false;
                    }
                ...
                case 2:
                    if (value == null)
                    {
                        errors = $"runtime error: string-type key \"{key}\" received null value";
                        return false;
                    }
                    break;
            }
            ...
            if (key == "scene")
            {
                ...
                {
                    case 0:
                        errors = $"runtime error: special key \"{key}\" received empty value";
                        return false;
                    case 1:
                        sceneFilter = '0' + value;
                        break;
                    ...
                    default:
                        ...
                        if (t.Length != 1)
                        {
                            errors = $"runtime error: special key \"{key}\" received an invalid value";
                            return false;
                        }
        ...
        private static bool? HandleFilter(PeroString peroString, MusicInfo musicInfo, KeyValuePair<string, string> filter)
        {
            if (filter.Key == null)
            {
                return null;
        ...
        private static bool EvalCinema(MusicInfo musicInfo)
        {
            return hasCinema.Contains(musicInfo.uid);
        }

        private static bool EvalAny(PeroString pStr, MusicInfo musicInfo, string filter)
        {
        ...
        private static bool EvalAnyX(PeroString pStr, MusicInfo musicInfo, string filter)
        {
        ...
        private static bool EvalTag(PeroString pStr, MusicInfo musicInfo, string filter)
        {
        ...
        private static bool EvalTitle(PeroString pStr, MusicInfo musicInfo, string filter)
        {
            ...
            if (AlbumManager.LoadedAlbumsByUid.ContainsKey(musicInfo.uid) && pStr.LowerContains(AlbumManager.LoadedAlbumsByUid[musicInfo.uid].Info.name_romanized ?? "", filter))
            {
                return true;
            }
        ...
        private static bool EvalAuthor(PeroString pStr, MusicInfo musicInfo, string filter)
        {
        ...
        private static bool EvalDesigner(PeroString pStr, MusicInfo musicInfo, string filter)
        {
        ...
        private static bool? EvalScene(MusicInfo musicInfo, string filter)
        {
            string sceneFilter = null;
        ...
        private static bool? EvalBPM(MusicInfo musicInfo, string filter)
        {
        ...
        private static bool EvalHidden(MusicInfo musicInfo, int type)
        {
            var musicDiff = musicInfo.GetMusicLevelStringByDiff(type, false);
            if (string.IsNullOrEmpty(musicDiff) || musicDiff == "0" || (AlbumManager.LoadedAlbumsByUid.ContainsKey(musicInfo.uid) && !AlbumManager.LoadedAlbumsByUid[musicInfo.uid].availableMaps.Keys.Contains(type)))
            {
                return false;
            }
            return true;
        ...
        private static bool? EvalHidden(MusicInfo musicInfo, string filter)
        {
            return EvalDiff(musicInfo, filter, 1);
        }

        
        private static bool? EvalDiff(MusicInfo musicInfo, string filter, byte type)
        {
            ...
            bool isCustom = AlbumManager.LoadedAlbumsByUid.ContainsKey(musicInfo.uid);
        ...
        private static bool EvalRange(string expression, out double start, out double end)
        {
            start = double.NaN;
            end = double.NaN;
